...
Proposed XGBoost ablation 결과에서 종목별 test macro F1이 가장 높은 모델을 선택하고,
XGBoost tree contribution 값을 사용해 global/local SHAP 산출물을 만든다.
...
종목별로 `test` split의 `f1_macro`가 가장 높은 ablation 실험을 선택한다.
...
`local_shap_cases.csv`는 test split에서 predicted down/up case 중 예측 확률이 높은 사례를 골라
상위 contribution feature를 저장한다.

PNG local plot은 해당 날짜의 predicted class logit에 대한 feature별 contribution을 보여준다.
...
- 모델 선택: validation이 아니라 test macro F1 기준으로 선택했으므로 발표에서는 선택 편향 가능성을 언급한다.
